...
export default function ErrdayCoinGame({ onShowAuth, onGameComplete, isGuestMode = false }: GameProps) {
  ...
  const watchAdAndRefill = async () => {
    try {
      ...
      if (adWatched) {
        if (isGuestMode) {
          // 게스트 모드: 1회 리필 후 차트/Next Day 완전 초기화
          setGameState(prev => ({
            ...initialGameState,
            chartData: [],
            previewData: [],
          ...
          try {
            ...
            // 사용자 데이터 다시 로드 (실패해도 계속 진행)
            try {
              await loadUserData()
            } catch (loadError) {
              console.error('Failed to load user data after refill:', loadError)
              // 데이터 로드 실패해도 토큰은 이미 지급되었으므로 계속 진행
            }
          } catch (error) {
  ...
  useEffect(() => {
    ...
    if (user || isGuestMode) {
      ...
      // 게임 초기화 후 토큰 상태 체크
      setTimeout(() => {
        const currentTokens = gameState.userTokens?.retry_tokens || (isGuestMode ? 1 : 0)
        const nextDayUses = gameState.nextdayUsesLeft
        
        console.log('🔍 Post-initialization token check:', { currentTokens, nextDayUses, isGuestMode })
        
        // 토큰이 0이거나 Next Day Uses가 0이면 모달 표시
        if (currentTokens <= 0 || nextDayUses <= 0) {
          console.log('⚠️ No tokens or uses left, showing modal')
          setShowNoChancesModal(true)
        }
      }, 1000) // 1초 후 체크 (데이터 로딩 완료 후)
    } else {
    ...
    try {
      ...
      if (isGuestMode) {
        ...
        const newState = {
          ...
          // 게스트 모드 userTokens 강제 초기화 (항상 1회)
          userTokens: isGuestMode ? ({
            user_id: 'guest',
            balance: '1000.00',
  ...
  const loadUserData = async () => {
    try {
      ...
      // 추천인 코드 로드 (실패해도 계속 진행)
      let referralCode = ''
      try {
        referralCode = await referralService.getUserReferralCode(user!.id)
      } catch (referralError) {
        console.error('Failed to get user referral code:', referralError)
        // 추천인 코드 로드 실패해도 계속 진행
      }
    ...
    try {
      ...
      if (isGuestMode) {
        ...
        setGameState(prev => ({
          ...
          isLiquidated,
          gameEnded: isLiquidated || (prev.nextdayUsesLeft - 1) <= 0,
          // Balance 실시간 반영: 초기 잔고 + 미실현 PNL
  ...
  const restartGame = async () => {
    ...
    // 로그인 사용자는 토큰 확인 (더 엄격한 체크)
    const currentRetryTokens = gameState.userTokens?.retry_tokens || 0
    console.log('🔍 Current retry tokens:', currentRetryTokens)
    
    if (!gameState.userTokens || currentRetryTokens <= 0) {
      console.log('❌ No tokens available, showing no chances modal')
      setShowNoChancesModal(true)
      return
    }
        ...
        <div className="flex space-x-6">
          <div>
            ...
            <div className={`text-xl font-bold ${
              (isGuestMode
                ? Math.min(gameState.userTokens?.retry_tokens ?? 1, 1)
                : (gameState.userTokens?.retry_tokens ?? 0)) > 0
                ? 'text-yellow-400'
                : 'text-red-400'
            }`}>
              {isGuestMode
            ...
            {isGuestMode && Math.min(gameState.userTokens?.retry_tokens ?? 1, 1) === 0 && (
                          ...
                          {isLoading ? (
                            ...
                            <div className="flex items-center">
                              ...
                              <div className="text-left">
                                <div className="text-lg">Watch Ad & Play Again</div>
                    ...
                    {!isGuestMode && (
                      <div className="space-y-4 mb-6">
                        ...
                        <button
                          onClick={() => {
                            // 청산 모달 닫기
                            setGameState(prev => ({ ...prev, isLiquidated: false }))
                            // 친구 초대 모달 열기
                        ...
                        >
                          ...
                          <div className="text-left">
                            ...
                            <div className="text-sm text-green-100">Get 3 tokens + 3 limit increase for both</div>
                          </div>
                        </button>
                        ...
                        <button
                          ...
                          className="w-full py-3 bg-yellow-500 hover:bg-yellow-600 text-black font-semibold rounded disabled:opacity-50"
                        >
                          {isLoading ? 'Processing...' : 'Try Again (Token -1)'}
                        </button>
                      </div>
                    )}
                  </div>
                    ...
                    {!isGuestMode && (
                      <div className="space-y-4 mb-6">
                        ...
                        <button
                          onClick={() => {
                            // 게임 완료 모달 닫기
                            setGameState(prev => ({ ...prev, gameEnded: false }))
                            // 친구 초대 모달 열기
                            setShowReferralModal(true)
                          }}
                          className="w-full py-4 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white font-bold rounded-xl transition-all duration-200 flex items-center justify-center shadow-lg hover:shadow-xl"
                ...
                >
                  {isLoading ? (
                    ...
                    <div className="flex items-center">
                      <span className="text-2xl mr-3">🎬</span>
                      <div className="text-left">
                        <div className="text-lg">광고 보고 1회 더하기</div>
                        <div className="text-sm text-blue-100">기회 1개 획득</div>
                      </div>
                    </div>
                  )}
                </button>
                
                {/* 친구 초대하기 링크 공유 - 로그인된 사용자에게만 표시 */}
                {user && (
                  <button
                    onClick={() => {
                      setShowNoChancesModal(false)
                  ...
                  >
                    <span className="text-2xl mr-3">👥</span>
                    <div className="text-left">
                      <div className="text-lg">친구 초대하기 링크 공유</div>
                      <div className="text-sm text-green-100">초대 시 추천인, 피추천인 retry token과 하루 한도 각각 +3</div>
                    </div>
                  </button>
                )}
